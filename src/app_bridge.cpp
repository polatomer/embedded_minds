...
{
    if (!chip || !outLine) return false;
    gpiod_line* line = gpiod_chip_get_line(chip, lineNumber);
    if (!line) { qWarning() << "GPIO line alinamadi:" << lineNumber; return false; }
    ...
    : QObject(parent)
{
    ...
    connect(&gasPollTimer_, &QTimer::timeout, this, &AppBridge::pollGasSensor);
    connect(&uiPollTimer_,  &QTimer::timeout, this, &AppBridge::pollUiInputs);
    ...
    if (voice_server_.start("aks_voice")) {
        ...
        qWarning() << "[AppBridge] Sesli komut sunucusu baslatılamadı";
    }
}
...
{
    qInfo() << "[VoiceCmd] Komut:" << command << "| Ekran:" << screen_;

    // ── Global komutlar ───────────────────────────────────────────────────────
    if (command == "ANA_SAYFA")        { goHome(); return; }
    if (command == "AYARLAR")          { openSettings(); return; }
    if (command == "VERITABANI")       { openRecords(); return; }
    if (command == "TURKCE")           { setLanguage("tr"); return; }
    if (command == "INGILIZCE")        { setLanguage("en"); return; }
    if (command == "AMBULANS_ARA")     { emit voiceAmbulansAraRequested(); return; }
    if (command == "YARDIM")           { emit voiceYardimRequested(); return; }
    if (command == "VERI_KAYDINI_SIL") { emit voiceVeriKaydiniSilRequested(); return; }

    if (command == "ACIL_DURUM") {
        if (screen_ == "home") startEmergency();
        return;
    }

    if (command == "GERI")    { handleBackButtonPressed(); return; }
    if (command == "DEVAM_ET") { handleEncoderPressed(); return; }

    if (command == "ILERI") {
        if (screen_ == "question") moveSelectionRight();
        else emit uiEncoderRotated(+1);
        return;
    }

    // ── Soru ekranı — cevap metni eşleştirme ─────────────────────────────────
    if (screen_ == "question") {
        if (command == "EVET") {
            int idx = findAnswerByNormalizedText("evet");
            submitAnswer(idx >= 0 ? idx : 0);
            return;
        }
        if (command == "HAYIR") {
            int idx = findAnswerByNormalizedText("hayir");
            if (idx < 0) idx = findAnswerByNormalizedText("no");
            submitAnswer(idx >= 0 ? idx : 1);
            return;
        }
        // Kanama bölgesi seçimi
        if (command == "KOL") {
            int idx = findAnswerByNormalizedText("kol");
            if (idx < 0) idx = findAnswerByNormalizedText("arm");
            if (idx >= 0) submitAnswer(idx);
            return;
        }
        if (command == "BACAK") {
            int idx = findAnswerByNormalizedText("bacak");
            if (idx < 0) idx = findAnswerByNormalizedText("leg");
            if (idx >= 0) submitAnswer(idx);
            return;
        }
        if (command == "VUCUT") {
            int idx = findAnswerByNormalizedText("vucut");
            if (idx < 0) idx = findAnswerByNormalizedText("govde");
            if (idx < 0) idx = findAnswerByNormalizedText("body");
            if (idx >= 0) submitAnswer(idx);
            return;
        }
        return;
    ...
    // ── Yönlendirme ekranları — popup'lara EVET/HAYIR ────────────────────────
    // main.qml her ekranın voiceYes() / voiceNo() fonksiyonunu çağırır
    if (command == "EVET") { emit voiceYesRequested(); return; }
    if (command == "HAYIR") { emit voiceNoRequested(); return; }

    // KOL/BACAK/VUCUT yönlendirme ekranında anlamsız — yoksay
    if (command == "KOL" || command == "BACAK" || command == "VUCUT") return;
...
{
    if (!current_question_.has_value()) return -1;

    const QStringList answers = currentAnswers();
    for (int i = 0; i < answers.size(); ++i) {
        QString a = answers.at(i).toLower().trimmed();
        a.replace(QChar(0x131), 'i')
         .replace(QChar(0x11F), 'g')
         .replace(QChar(0xFC),  'u')
         .replace(QChar(0x15F), 's')
         .replace(QChar(0xF6),  'o')
         .replace(QChar(0xE7),  'c');
...
QString AppBridge::screen() const { return screen_; }

void AppBridge::setScreen(const QString& screenName)
{
    if (screen_ == screenName) return;
    screen_ = screenName;
    emit screenChanged();
}
...
{
    if (!initialized_) { emit errorOccurred("Önce veri yüklenmeli."); return; }
    cpr_controller_.stop();
    bleeding_controller_.stop();
    event_recorder_.logDecision("cpr_pulse_check", "Nabız ve solunum var - soru ekranına dönüldü", "kullanici_onayladi");
    session_ = Session{};
    ...
    if (session_.finished) {
        clearQuestionState(); updateResultState(); openFinalScreen();
        emit questionChanged(); emit guidanceChanged(); emit screenChanged();
        return;
    }
    selected_answer_index_ = 0; emit selectedAnswerIndexChanged();
    updateQuestionState();
    screen_ = "question";
    emit questionChanged(); emit guidanceChanged(); emit screenChanged();
}
...
{
    cpr_controller_.stop(); bleeding_controller_.stop();
    event_recorder_.logGuidance("cpr", "vital_alert_nabiz_cok_dusuk");
    screen_ = "cpr"; emit screenChanged();
}
...
void AppBridge::navigateToLowSpo2()
{
    ...
    screen_ = "low_spo2"; emit screenChanged();
}
...
QString AppBridge::language() const { return language_; }
...
void AppBridge::setLanguage(const QString& lang)
{
    ...
    language_ = lang;
    session_.language = language_.toStdString();
    emit languageChanged(); emit questionChanged(); emit guidanceChanged();
}

QString AppBridge::mediaBaseUrl() const { return media_base_url_; }
...
{
    if (!current_question_.has_value()) return "";
    return QString::fromStdString(Core::pick_text(current_question_->text, session_.language));
}

QString AppBridge::currentQuestionHint() const
{
    if (!current_question_.has_value()) return "";
    return QString::fromStdString(Core::pick_text(current_question_->hint, session_.language));
}

QStringList AppBridge::currentAnswers() const
{
    QStringList result;
    if (!current_question_.has_value()) return result;
    for (const auto& ans : current_question_->answers)
        result << QString::fromStdString(Core::pick_text(ans.text, session_.language));
    return result;
}
...
int AppBridge::selectedAnswerIndex() const { return selected_answer_index_; }
...
{
    if (!final_diagnosis_.has_value()) return "";
    return QString::fromStdString(Core::pick_text(final_diagnosis_->name, session_.language));
}

QString AppBridge::finalScreenId() const
{
    return final_screen_id_.has_value() ? QString::fromStdString(*final_screen_id_) : "";
}

QString AppBridge::finalProtocolId() const
{
    return final_protocol_id_.has_value() ? QString::fromStdString(*final_protocol_id_) : "";
}
...
void AppBridge::onSensorChanged()
{
    ...
    const int  bpm = pulseDetected ? static_cast<int>(std::round(vital_signs_.heartRateBpm())) : 0;
    const bool spo2Stable = vital_signs_.signalStable();
    const int  spo2 = spo2Stable ? static_cast<int>(std::round(vital_signs_.spo2())) : 0;
...
bool AppBridge::initialize(const QString& basePath)
{
    try {
        ...
        session_ = Session{};
        session_.language = language_.toStdString();
        if (!initialized_) emit errorOccurred("Veriler yüklenemedi.");
        return initialized_;
...
void AppBridge::goHome()
{
    ...
    session_ = Session{};
    session_.language = language_.toStdString();
    clearQuestionState(); clearResultState();
    selected_answer_index_ = 0; emit selectedAnswerIndexChanged();
    screen_ = "home"; emit screenChanged();
}

void AppBridge::openSettings()
{
    cpr_controller_.stop(); bleeding_controller_.stop();
    screen_ = "settings"; emit screenChanged();
}

void AppBridge::openRecords()
{
    cpr_controller_.stop(); bleeding_controller_.stop();
    screen_ = "records"; emit screenChanged();
}

void AppBridge::openEventDetail(const QString& eventId)
{
    Q_UNUSED(eventId)
    screen_ = "event_detail"; emit screenChanged();
}
...
{
    if (!current_question_.has_value()) return;
    if (index < 0 || index >= static_cast<int>(current_question_->answers.size())) return;
    ...
    SubmitResult result = core_.submit_answer(
        session_, current_question_->id, current_question_->answers[index].id);
    ...
    selected_answer_index_ = 0; emit selectedAnswerIndexChanged();
    updateQuestionState(); emit questionChanged();
}

void AppBridge::previousQuestion()
{
    if (screen_ != "question") return;
    std::optional<Question>     previousQuestionValue;
    std::optional<std::string>  previousAnswerId;
    if (!core_.go_to_previous_question(session_, previousQuestionValue, previousAnswerId)) return;
    ...
    emit selectedAnswerIndexChanged(); emit questionChanged(); emit guidanceChanged();
}

void AppBridge::moveSelectionLeft()
{
    if (screen_ != "question" || !current_question_.has_value()) return;
    if (selected_answer_index_ > 0) { selected_answer_index_--; emit selectedAnswerIndexChanged(); }
}
...
void AppBridge::moveSelectionRight()
{
    ...
    if (selected_answer_index_ < maxIndex) { selected_answer_index_++; emit selectedAnswerIndexChanged(); }
}
...
void AppBridge::clearQuestionState()  { current_question_.reset(); }
void AppBridge::clearResultState()    { final_diagnosis_.reset(); final_screen_id_.reset(); final_protocol_id_.reset(); }
...
{
    if (!current_question_.has_value()) return 0;
    for (int i = 0; i < static_cast<int>(current_question_->answers.size()); ++i)
        if (current_question_->answers[i].id == answerId) return i;
    return 0;
}
...
{
    if (!setupGasGpio()) { emit errorOccurred("Gaz modulu GPIO baslatilamadi."); return; }
    gasPollTimer_.setInterval(kGasPollMs); gasPollTimer_.start();
}

void AppBridge::stopGasMonitor()
{
    gasPollTimer_.stop(); releaseLine(gasLine_); closeChip(gasChip_);
}
...
bool AppBridge::setupGasGpio()
{
    ...
    if (!gasChip_) { qWarning() << "gpiod_chip_open basarisiz"; return false; }
    gasLine_ = gpiod_chip_get_line(gasChip_, kGasBcmLine);
    if (!gasLine_) { qWarning() << "GPIO line alinamadi:" << kGasBcmLine; closeChip(gasChip_); return false; }
    gpiod_line_request_config config = {};
    ...
    if (gpiod_line_request(gasLine_, &config, 0) < 0) {
        ...
        gasLine_ = nullptr; closeChip(gasChip_); return false;
    }
    gasDetectedLast_ = false; lastGasAlertMs_ = 0;
    return true;
}
...
void AppBridge::pollGasSensor()
{
    ...
    if (raw < 0) { qWarning() << "Gaz GPIO okunamadi"; return; }
    const bool detected = kGasActiveLow ? (raw == 0) : (raw == 1);
    const qint64 nowMs  = QDateTime::currentMSecsSinceEpoch();
    if (detected && !gasDetectedLast_)
        if ((nowMs - lastGasAlertMs_) >= kGasCooldownMs) { playGasAlert(); lastGasAlertMs_ = nowMs; }
    gasDetectedLast_ = detected;
}

void AppBridge::playGasAlert()
{
    const QString audioPath = QDir::currentPath() + QString::fromUtf8(kGasAlertAudioRelativePath);
    if (!QFileInfo::exists(audioPath)) { emit errorOccurred("Gaz alarm ses dosyasi bulunamadi: " + audioPath); return; }
...
{
    if (!setupUiInputGpio()) { emit errorOccurred("Buton veya rotary GPIO baslatilamadi."); return; }
    uiPollTimer_.setInterval(kUiPollMs); uiPollTimer_.start();
}

void AppBridge::stopUiInputMonitor()
{
    uiPollTimer_.stop();
    releaseLine(homeButtonLine_); releaseLine(backButtonLine_);
    releaseLine(encoderClkLine_); releaseLine(encoderDtLine_);
    releaseLine(encoderSwLine_); closeChip(uiChip_);
}
...
bool AppBridge::setupUiInputGpio()
{
    ...
    if (!uiChip_) { qWarning() << "ui gpiochip acilamadi"; return false; }
    ...
    if (homeRaw < 0 || backRaw < 0 || swRaw < 0 || clkRaw < 0 || dtRaw < 0) {
        qWarning() << "baslangic gpio degerleri okunamadi"; stopUiInputMonitor(); return false;
    }
    ...
    encoderClkLastRaw_ = clkRaw; encoderDtLastRaw_ = dtRaw;
    lastHomeButtonMs_ = 0; lastBackButtonMs_ = 0; lastEncoderPressMs_ = 0; lastEncoderStepMs_ = 0;
    return true;
}

void AppBridge::pollUiInputs()
{
    if (!homeButtonLine_ || !backButtonLine_ || !encoderClkLine_ || !encoderDtLine_ || !encoderSwLine_) return;
    const int homeRaw = gpiod_line_get_value(homeButtonLine_);
    const int backRaw = gpiod_line_get_value(backButtonLine_);
    const int clkRaw  = gpiod_line_get_value(encoderClkLine_);
    const int dtRaw   = gpiod_line_get_value(encoderDtLine_);
    const int swRaw   = gpiod_line_get_value(encoderSwLine_);
    if (homeRaw < 0 || backRaw < 0 || clkRaw < 0 || dtRaw < 0 || swRaw < 0) { qWarning() << "ui gpio okunamadi"; return; }
    const qint64 nowMs     = QDateTime::currentMSecsSinceEpoch();
    ...
    if (homePressed && !homeButtonLastPressed_ && (nowMs - lastHomeButtonMs_) >= kButtonDebounceMs)
        { handleHomeButtonPressed(); lastHomeButtonMs_ = nowMs; }
    if (backPressed && !backButtonLastPressed_ && (nowMs - lastBackButtonMs_) >= kButtonDebounceMs)
        { handleBackButtonPressed(); lastBackButtonMs_ = nowMs; }
    if (swPressed && !encoderSwLastPressed_ && (nowMs - lastEncoderPressMs_) >= kEncoderPressDebounceMs)
        { handleEncoderPressed(); lastEncoderPressMs_ = nowMs; }
    if (clkRaw != encoderClkLastRaw_) {
        ...
        if (fallingEdge && (nowMs - lastEncoderStepMs_) >= kEncoderStepDebounceMs) {
            if (dtRaw != clkRaw) handleEncoderRotate(+1); else handleEncoderRotate(-1);
...
void AppBridge::handleHomeButtonPressed() { goHome(); }

void AppBridge::handleBackButtonPressed()
{
    if (screen_ == "question") { previousQuestion(); return; }
    emit uiBackRequested();
}

void AppBridge::handleEncoderPressed()
{
    if (screen_ == "question") { activateSelectedAnswer(); return; }
...
{
    if (screen_ == "question") {
        if (direction > 0) moveSelectionRight();
        else if (direction < 0) moveSelectionLeft();
        return;
    }
    if (direction != 0) emit uiEncoderRotated(direction);
}
